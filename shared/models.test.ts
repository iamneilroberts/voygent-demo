...
describe("enabledModels", () => {
  it("excludes Opus unless enabled", () => {
    expect(enabledModels(false)).toEqual(["claude-haiku-4-5", "claude-sonnet-4-6"]);
    expect(enabledModels(true)).toContain("claude-opus-4-8");
  });
});

describe("coerceModel", () => {
  const enabled = enabledModels(false);
...
describe("resolveRoutingModel", () => {
  it("single mode ignores phase", () => {
    const r = buildRouting({ model: "claude-haiku-4-5" }, enabledModels(false), "claude-sonnet-4-6");
  ...
  it("smart mode picks by milestone", () => {
    const r = buildRouting(undefined, enabledModels(false), "claude-sonnet-4-6");
