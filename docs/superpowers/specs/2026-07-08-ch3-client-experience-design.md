...
6. **C11 redefined (2026-07-08 post-ship, Neil).** The literal "swap FolioArtifact → ReelFolioView" is retracted: FolioArtifact (`ClaudeChatView.tsx`) is the **advisor-view** chat folio — commission chips, bookings, edit markers, comment threads, and ch1/ch2 spotlight anchors (`folio-bookings`, `trip-commission`, 0-based `folio-day-${i}`) live there — so it must not become the client window. C11 splits into two steps in this lane: **(a) retire ReelExplore** into ReelFolioView interactive mode (adapter `ReelClientSession`+canonical folio → `ReelFolioSession`; ReelFolioView gains a hotel chooser, an undated-add-ons section, and the send-to-Voygent funnel), then **(b) restyle FolioArtifact** to the alaska-warm production look, keeping its structure and anchors intact.

7. **C9 shape: scripted `folioview` cutaway (2026-07-08, Neil — "cutaway").** Ch1 and ch2 each gain one short mid-chapter cutaway to ReelFolioView scripted mode (2–3 snapshots + one spotlight, then close with `folioView(null)`): ch1 right after the Act-7 send (the folio as it lands — hero, then the hotel chooser the shortlist became), ch2 right after the Wicklow pick (day 6 already current on the Millers' page). Chosen over the quick spotlight-style pop-up. Constraints: cutaway spotlights may only target anchors ReelFolioView emits and FolioArtifact does NOT (`folio-hero`, `folio-hotel-choice`, `folio-total`, `folio-day-6`-of-6…) because ReelCallout resolves `data-reel-target` globally and the chat folio stays in the DOM behind the scrim — guarded by test. Every cutaway must end closed so the ended branch keeps deriving ch1/ch2 end-states from `clientView` (the both-views-set trap) — guarded by test.
...
1. **Visual contract first:** screenshot a real alaska-warm folio from staging (`preview_folio` output for a comparable trip) into `docs/reference/` (or the spec dir) before styling. "Looks like production" is checked against that artifact, not memory. Contract artifact: `docs/reference/2026-07-08-alaska-warm-folio-staging.png` (captured 2026-07-08, staging trip `horn-alaska-princess-2026`). Key traits: cream page over deep-pine serif hero; amber pill badge; serif section headings with hairline rule; day cards = dark-green numbered circle + tan header band + white body cards; optional add-ons as white cards with orange price and green "Optional" chip; thin green-bordered stat tiles.
