...
**Gate decision (owner: Neil):** ship Plan A **flag-off**. `FAITHFUL=1` alone enables faithful mode for **test/admin runs only** (the `x-demo-test` path, which skips the public ledger) — safe to exercise now. The public switch `FAITHFUL_PUBLIC_OK=1` originally awaited a real-spend backstop (per-tool-call cost estimate, per-code round cap, or supplier-side limits).

**UPDATE 2026-06-09 — gate WAIVED for current scale; public live faithful APPROVED (Neil).** Neil accepts the unmetered real-supplier spend at the planned (low) scale: alerts/limits are set, heavy public use is not anticipated near-term, and anyone given an access token is welcome to use it freely. Verification done: (1) `liveMode` already makes real, unmetered supplier calls for off-menu destinations today (`session-do.ts:454-457`) — faithful generalizes an existing property rather than introducing a new one; (2) the demo's MCP bearer hits the advisor's **prod, globally-shared** SerpAPI/TripAdvisor/Viator keys (confirmed in voygent-lite) — accepted for now. Supplier-key isolation is deferred to **issue iamneilroberts/voygent-lite#179** (revisit only if driving more public traffic). So both flags are now OK to set, in this order: `FAITHFUL=1` + Task 6 smoke first, then `FAITHFUL_PUBLIC_OK=1` for public.
